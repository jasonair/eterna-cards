...
export default function InventoryPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-stone-900 overflow-hidden">
      {/* Sticky top section */}
      <div className="flex-none px-3 sm:px-4 lg:px-6 pt-4 sm:pt-5 pb-3 space-y-3 sm:space-y-4 border-b border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900">
        {/* Header */}
        <div className="flex flex-row items-center justify-between gap-2">
          <div>
            <p className="text-[11px] sm:text-xs font-semibold uppercase tracking-[0.2em] text-red-600">Operations Console</p>
            <h1 className="text-2xl sm:text-4xl font-bold tracking-tight text-stone-900 dark:text-stone-100">Inventory</h1>
            <p className="text-xs sm:text-sm text-stone-500 dark:text-stone-400 mt-1">Realtime stock, folders, and product flow in one workspace.</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleRefresh}
              title="Refresh"
              className="inline-flex items-center justify-center w-10 h-10 border border-transparent rounded-xl text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-600 transition-colors"
            >
        ...
        {/* Global error */}
        {error && (
          <div className="bg-white border border-stone-200 rounded-lg p-3 text-sm text-red-700">
            {error}
          </div>
        )}
        ...
        <div className={`transition-all duration-300 ease-in-out sm:block ${headerScrolled ? 'max-h-0 overflow-hidden opacity-0 sm:max-h-none sm:overflow-visible sm:opacity-100 -mt-3 sm:mt-0' : 'max-h-56 overflow-visible opacity-100'}`}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 px-0.5 pb-0.5">
            <button
              ...
              className={`flex flex-col justify-between py-1 text-left border-b-2 transition-colors ${stockFilter === 'all'
                ? 'border-red-600 text-red-600 dark:text-red-400'
                : 'border-transparent text-stone-900 dark:text-stone-100 hover:text-red-600 dark:hover:text-red-400'
                }`}
            >
              <p className={`text-[10px] sm:text-xs font-semibold tracking-[0.16em] uppercase ${stockFilter === 'all' ? 'text-red-600 dark:text-red-400' : 'text-stone-500 dark:text-stone-400'}`}>Products</p>
              {loading ? (
                <div className="h-6 sm:h-7 w-12 bg-stone-100 dark:bg-stone-700 rounded animate-pulse mt-1" />
              ) : (
                <p className={`text-xl sm:text-2xl font-bold mt-1 ${stockFilter === 'all' ? 'text-red-600 dark:text-red-400' : 'text-stone-900 dark:text-stone-100'}`}>{items.length.toLocaleString()}</p>
              )}
            </button>
            <button
              type="button"
              onClick={() => setStockFilter('onHand')}
              className={`flex flex-col justify-between py-1 text-left border-b-2 transition-colors ${stockFilter === 'onHand'
                ? 'border-red-600 text-red-600 dark:text-red-400'
                : 'border-transparent text-stone-900 dark:text-stone-100 hover:text-red-600 dark:hover:text-red-400'
                }`}
            >
              <p className={`text-[10px] sm:text-xs font-semibold tracking-[0.16em] uppercase ${stockFilter === 'onHand' ? 'text-red-600 dark:text-red-400' : 'text-stone-500 dark:text-stone-400'}`}>In Hand</p>
              {loading ? (
                <div className="h-6 sm:h-7 w-12 bg-stone-100 dark:bg-stone-700 rounded animate-pulse mt-1" />
              ) : (
                <p className={`text-xl sm:text-2xl font-bold mt-1 ${stockFilter === 'onHand' ? 'text-red-600 dark:text-red-400' : 'text-stone-900 dark:text-stone-100'}`}>
                  £{items.reduce(
            ...
            <button
              ...
              className={`flex flex-col justify-between py-1 text-left border-b-2 transition-colors ${stockFilter === 'inTransit'
                ? 'border-red-600 text-red-600 dark:text-red-400'
                : 'border-transparent text-stone-900 dark:text-stone-100 hover:text-red-600 dark:hover:text-red-400'
                }`}
            >
              <p className={`text-[10px] sm:text-xs font-semibold tracking-[0.16em] uppercase ${stockFilter === 'inTransit' ? 'text-red-600 dark:text-red-400' : 'text-stone-500 dark:text-stone-400'}`}>In Transit</p>
              {loading ? (
                <div className="h-6 sm:h-7 w-12 bg-stone-100 dark:bg-stone-700 rounded animate-pulse mt-1" />
              ) : (
                <p className={`text-xl sm:text-2xl font-bold mt-1 ${stockFilter === 'inTransit' ? 'text-red-600 dark:text-red-400' : 'text-stone-900 dark:text-stone-100'}`}>
            ...
            <div className="flex flex-col justify-between py-1 text-left">
              <p className="text-[10px] sm:text-xs font-semibold tracking-[0.16em] text-stone-500 dark:text-stone-400 uppercase">Total Value</p>
              {loading ? (
                <div className="h-6 sm:h-7 w-24 bg-stone-100 dark:bg-stone-700 rounded animate-pulse mt-1" />
              ) : (
                <p className="text-xl sm:text-2xl font-bold text-stone-900 dark:text-stone-100 mt-1">
                  £{items.reduce(
          ...
          <div className="flex items-center gap-2 flex-1 sm:flex-none sm:ml-auto sm:order-2">
            <div className="relative flex-1 sm:max-w-xs sm:w-72">
              <input
                ...
                className="w-full rounded-md bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-stone-900 dark:text-stone-100 text-xs px-3 py-1.5 pr-7 focus:outline-none focus:ring-2 focus:ring-red-600"
              />
              {search ? (
                <button
                  type="button"
            ...
            <button
              ...
              className="inline-flex items-center justify-center w-8 h-8 rounded-md bg-red-600 text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-600 sm:hidden flex-shrink-0"
            >
            ...
            <button
              ...
              className="hidden sm:block px-3 py-1.5 text-xs rounded-md text-stone-900 dark:text-stone-100 hover:text-red-600 dark:hover:text-red-400 whitespace-nowrap"
            >
              New item
            </button>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 sm:order-1 sm:flex-shrink-0">
              ...
              {activeFolderPath.map((segment, index) => (
                <span key={segment.id} className="flex items-center gap-1.5 min-w-0">
                  ...
                  {index === activeFolderPath.length - 1 ? (
                    <span className="px-2 py-0.5 rounded-md bg-red-600 text-white font-semibold truncate max-w-[120px]">
                      {segment.name}
                    </span>
                  ) : (
            ...
            <div className="flex items-center gap-1.5 sm:hidden flex-shrink-0">
              ...
              <button
                type="button"
                onClick={() => router.push('/inventory/new')}
                className="px-2.5 py-1.5 text-xs rounded-md text-stone-900 dark:text-stone-100 hover:text-red-600 dark:hover:text-red-400 whitespace-nowrap"
              >
                New item
              </button>
            </div>
      ...
      {/* Scrollable content */}
      <div className="flex-1 overflow-hidden w-full pb-0 px-3 sm:px-4 lg:px-6 md:pl-0">
        <div className="h-full w-full flex gap-3">
              ...
              <div className="px-3 py-2 border-b border-stone-200 dark:border-stone-700 flex items-center justify-between bg-white dark:bg-stone-900">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="inline-flex items-center justify-center h-7 w-7 rounded-md border border-stone-200 dark:border-stone-700 text-stone-800 dark:text-stone-200 bg-stone-50 dark:bg-stone-800 hover:bg-white dark:hover:bg-red-900/30 text-xs"
                  >
                    +
                  </button>
                  <button
              ...
              <div className="flex-1 overflow-y-auto overflow-x-hidden py-2">
                {isCreatingFolder && (
                  <div className="px-3 pb-2 flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 rounded-md bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-[11px] text-stone-900 dark:text-stone-100 px-2 py-1 focus:outline-none focus:ring-1 focus:ring-red-600"
                    />
                    <button
                      type="button"
                      onClick={handleCreateFolder}
                      className="px-2 py-1 text-[11px] rounded-md bg-red-600 text-white hover:bg-red-700"
                    >
                      Save
                    </button>
                    <button
                ...
                <button
                  ...
                  className={`flex items-center justify-between w-full px-4 py-2.5 text-sm font-medium border-l-2 transition-colors transition-transform duration-150 ${activeFolderId === 'all' || dragOverFolderId === 'all'
                    ? 'border-red-600 bg-stone-50 dark:bg-stone-800 text-stone-900 dark:text-stone-100 translate-x-0.5'
                    : isDraggingFolder
                ...
                >
                  ...
                  {dragOverFolderId === 'all' ? (
                    <span className="ml-2 text-[10px] text-red-500">Drop to move folder to top level</span>
                  ) : isDraggingFolder ? (
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between w-full py-2.5 border-l-2 transition-colors transition-transform duration-150 ${activeFolderId === folder.id || dragOverFolderId === folder.id
                            ? 'border-red-600 bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-stone-100 translate-x-0.5'
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-stone-600 pr-2">
                            {isCustom && hasChildren && (
                              <button
                                ...
                                className="inline-flex items-center justify-center h-5 w-5 rounded hover:bg-white focus:outline-none"
                            ...
                            {dragOverFolderId === folder.id && (
                              <span className="px-2 py-0.5 rounded-full bg-white text-red-500 border border-stone-200">
                                Drop to move here
                              </span>
                            )}
                            {isCustom && (
                              <button
                                ...
                                className="ml-1 inline-flex items-center justify-center h-5 w-5 rounded-full text-xs text-stone-400 hover:text-red-500 hover:bg-white disabled:opacity-50"
          ...
          <div className="flex-1 overflow-y-auto w-full bg-white dark:bg-stone-900 p-3 sm:p-4 lg:p-5" onScroll={(e) => {
            ...
            {loading ? (
              ...
              <div className="p-10 text-center text-sm text-stone-600 dark:text-stone-400 border-t border-stone-200 dark:border-stone-700">
                <div className="mx-auto mb-4 h-12 w-12 rounded-md bg-red-600 text-white flex items-center justify-center">
                  <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-6 4h6M5 7h14M7 3h10a2 2 0 012 2v14a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z" />
                  </svg>
                </div>
                <p className="text-base font-semibold text-stone-800 dark:text-stone-200">No products match this search or folder.</p>
                <p className="text-xs text-stone-500 mt-1">Try a different filter or import products to get started.</p>
              </div>
            ) : viewMode === 'table' ? (
              <div className="overflow-hidden border-t border-stone-200 dark:border-stone-700">
                  ...
                  <tbody className="divide-y divide-stone-200 dark:divide-stone-700">
                    {visibleItems.map((row) => {
                      return (
                        <tr
                          key={row.product.id}
                          className="hover:border-l-2 hover:border-l-red-600 dark:hover:bg-stone-700/20 cursor-pointer transition-colors"
                          onClick={() => router.push(`/inventory/${row.product.id}`)}
                        >
                          <td className="px-4 py-3 text-sm text-stone-900 dark:text-stone-100">
                            <div className="flex items-center gap-3">
                              <div className="h-9 w-9 rounded-md bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-[10px] font-bold text-stone-800 dark:text-stone-200 uppercase flex-shrink-0">
                                {row.product.name
                                  .split(' ')
                                  .filter(Boolean)
                                  .slice(0, 2)
                          ...
                          <td className="px-4 py-3 text-sm text-right">
                            <button
                              ...
                              className="inline-flex items-center justify-center p-1.5 rounded-md text-stone-400 hover:text-red-500 hover:bg-white disabled:opacity-50"
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 ${foldersPanelCollapsed ? 'lg:grid-cols-3 xl:grid-cols-4' : 'xl:grid-cols-3'
                ...
                {visibleItems.map((row) => {
                  ...
                  return (
                    <div
                      key={row.product.id}
                      className="group bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-lg overflow-hidden cursor-pointer hover:border-red-600 dark:hover:border-red-600 transition-all duration-200 flex flex-col"
                      onClick={() => router.push(`/inventory/${row.product.id}`)}
                      draggable
                      onDragStart={(e) => handleProductDragStart(e, row.product)}
                    >
                      <div className="h-28 bg-stone-100 dark:bg-stone-700 flex items-center justify-center border-b border-stone-100 dark:border-stone-700 overflow-hidden">
                      ...
                      <div className="p-3 flex flex-col flex-1">
                        <div className="flex items-start justify-between gap-2 flex-1 mb-2">
                          ...
                          <button
                            ...
                            disabled={deletingProductId === row.product.id}
                            className="opacity-0 group-hover:opacity-100 p-1 rounded text-stone-300 hover:text-red-500 hover:bg-white transition-all disabled:opacity-50 flex-shrink-0"
                            aria-label="Delete product"
                          >
                            {deletingProductId === row.product.id ? (
                        ...
                        <div className="flex items-baseline gap-4 pt-2 mt-2 border-t border-stone-100 dark:border-stone-700">
                          ...
                          <div>
                            <span className="text-sm font-medium text-red-600 tabular-nums">{inTransit}</span>
                            <span className="text-[10px] text-red-500 ml-1 uppercase tracking-wide">in transit</span>
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 max-w-xs px-3 py-2 rounded-md bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-xs text-stone-900 dark:text-stone-100">
          {toastMessage}
        </div>
      )}
      ...
      <div className={`fixed inset-0 z-50 md:hidden transition-all duration-300 ${mobileFoldersOpen ? 'visible' : 'invisible'}`}>
        ...
        />
        {/* Drawer */}
        <div className={`absolute inset-y-0 left-0 w-72 flex flex-col bg-white dark:bg-stone-900 transition-transform duration-300 ease-in-out ${mobileFoldersOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden py-2">
            {isCreatingFolder && (
              <div className="px-3 pb-2 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded-md bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-[11px] text-stone-900 dark:text-stone-100 px-2 py-1 focus:outline-none focus:ring-1 focus:ring-red-600"
                />
                <button type="button" onClick={handleCreateFolder} className="px-2 py-1 text-[11px] rounded-md bg-red-600 text-white hover:bg-red-700">Save</button>
            ...
            <button
              ...
              className={`flex items-center justify-between w-full px-4 py-2.5 text-sm font-medium border-l-2 transition-colors ${activeFolderId === 'all'
                ? 'border-red-600 bg-stone-50 dark:bg-stone-800 text-stone-900 dark:text-stone-100'
                : 'border-transparent text-stone-800 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800'
                }`}
            >
              <span className="flex items-center gap-2">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between w-full py-2.5 border-l-2 transition-colors ${activeFolderId === folder.id
                        ? 'border-red-600 bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-stone-100'
                        : 'border-transparent text-stone-600 dark:text-stone-400 hover:bg-stone-100 dark:hover:bg-stone-800'
                        }`}
                      style={{ paddingLeft }}
                    >
                      ...
                      <div className="flex items-center gap-1 pr-2">
                        {isCustom && hasChildren && (
                          <button type="button" onClick={(e) => { e.stopPropagation(); toggleFolderCollapsed(folder.id); }} className="inline-flex items-center justify-center h-5 w-5 rounded hover:bg-white">
                        ...
                        {isCustom && (
                          <button type="button" onClick={() => handleDeleteFolder(folder.id)} disabled={deletingFolderId === folder.id} className="inline-flex items-center justify-center h-5 w-5 rounded-full text-xs text-stone-400 hover:text-red-500 hover:bg-white disabled:opacity-50">×</button>
                        )}
                      </div>
                    </div>
                  );
